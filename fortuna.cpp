// Compile with: "cl /EHsc fortuna.cpp User32.lib"
...
// From: https://joshmadison.com/2008/04/20/fortune-cookie-fortunes/
const char* quotes[] = {
...
char* readClipboard(void) {
    ...
    char* returnedBuffer = static_cast<char*>(malloc(strlen(clipboardText) + 1));
...
bool writeClipboard(const char* clipboardText) {
...
bool pressOriginalKey(int hotkeyID, WORD vk) {
    // Temporarly disable hotkey and block user input.
    if (!UnregisterHotKey(NULL, hotkeyID))
        return true;
    BlockInput(true);
    ...
    ip.ki.wScan = 0; ip.ki.time = 0;
    ip.ki.dwExtraInfo = 0;

    // Press the "Ctrl" key.
    ip.ki.wVk = VK_CONTROL; ip.ki.dwFlags = 0;
    SendInput(1, &ip, sizeof(INPUT));

    // Press the specified key.
    ip.ki.wVk = vk; ip.ki.dwFlags = 0;
    SendInput(1, &ip, sizeof(INPUT));

    // Release the specified key.
    ip.ki.wVk = vk; ip.ki.dwFlags = KEYEVENTF_KEYUP;
    SendInput(1, &ip, sizeof(INPUT));

    // Release the "Ctrl" key.
    ip.ki.wVk = VK_CONTROL; ip.ki.dwFlags = KEYEVENTF_KEYUP;
    SendInput(1, &ip, sizeof(INPUT));

    // Re-enable hotkey and input.
    if (!RegisterHotKey(NULL, hotkeyID, MOD_CONTROL, vk))
        return true;
    BlockInput(false);
    return false;
}
...
int main(void) {
    // Initialize the random engine, hide the console and
    // register to receive CTRL+V keypress messages.
    srand(time(0));
    if (hideWindow() || !RegisterHotKey(NULL, 3, MOD_CONTROL, 0x56))
        return -1;
    ...
    while (GetMessage(&msg, NULL, 0, 0) != 0) {
        ...
        if (!done) {
            clipboardBuf = readClipboard();
            if (writeClipboard(quotes[rand() % sizeof(quotes) / sizeof(char*)]))
                return -1;

        } else {
            if (writeClipboard(clipboardBuf))
                return -1;

            // Was malloc'd in readClipboard()
            free(clipboardBuf);
        }
